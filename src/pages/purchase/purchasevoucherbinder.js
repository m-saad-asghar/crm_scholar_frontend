...
const Page = () => {
  ...
  const addPurchase = () => {
    ...
    fetch(baseUrl + 'add_new_voucher_lamination', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
if(batchno != 0 && process != 0){
  fetch(baseUrl + 'get_batch_data_for_binder_pv/' + batchno + '/' + process,{     
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
setPrintOrder(data.batchData[0]['printOrder']);
setReceivedQty(data.batchData[0]['receivedQty']);
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h6" component="h2">
            + Add Purchase Voucher For Binder
          </Typography>
          <Typography id="modal-modal-description" sx={{ mt: 4 }}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={3} md={3} lg={3}>
                ...
                <TextField id="received_qty" aria-describedby="add-received_qty"
...
{tableData.map((rowData, index) => (
  ...
  <td>{rowData.product_amount}</td>
      ...
      <Head>
        <title>
          Purchase Voucher For Binder | Scholar CRM
        </title>
      </Head>
      <Box
        component="main"
