...
if(batchno != 0 && process != 0){
  fetch(baseUrl + 'get_batch_data_for_press_pv/' + batchno + '/' + process,{     // it also can be used for lamination
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h6" component="h2">
            + Add Purchase Voucher For Lamination
          </Typography>
...
{tableData.map((rowData, index) => (
  ...
  <td>{rowData.product_amount}</td>
      ...
      <Head>
        <title>
          Purchase Voucher For Lamination | Scholar CRM
        </title>
      </Head>
      <Box
        component="main"
            ...
            >
              <Stack spacing={1}>
                <Typography variant="h4">
                  Purchase Voucher For Lamination
                </Typography>
                {/*<Stack*/}
                {/*  alignItems="center"*/}
                {/*  direction="row"*/}
